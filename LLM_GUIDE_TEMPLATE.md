...
This directory (`games/{{GAME_NAME}}`) is your isolated project workspace. The MIX Engine is currently running and watching this folder. Any changes you make here will be instantly reflected in the live engine.
...
### Build the world with one command

For a new map, use `world_compose` before hand-authoring hundreds of terrain operations. It creates
the terrain/biomes, traversal layout, water, foliage, weather, cinematic lighting, and navigation as
one deterministic recipe. Use `balanced` while iterating and `aaa` for the final bake.
...
```json
{
  ...
  "quality": "balanced",
  "pointsOfInterest": [
    { "name": "harbor", "kind": "settlement", "x": -140, "z": 80, "radius": 35 },
    { "name": "temple", "kind": "landmark", "x": 120, "z": -100, "radius": 28 }
  ],
  "paths": [
    { "name": "coast_road", "kind": "road", "width": 12,
      "points": [{ "x": -140, "z": 80 }, { "x": -20, "z": 10 }, { "x": 120, "z": -100 }] }
  ]
}
```

### Safe agent edit loop

For authored changes, prefer HELM's guarded workflow:

1. Call `mix_plan` (or `node scripts/mix-helm.js plan --file edits.json --atomic`).
2. Call `mix_apply` with postconditions and a stable `requestKey` for that logical edit.
3. Call `mix_observe` to verify the rendered pixels, not only scene state.

Never persist runtime numeric entity ids in authored commands. Use `@exactName`, `guid:<guid>`, or a
unique `tag:<tag>` in fields such as `entityId`, `parentId`, `rootEntity`, `entityA`/`entityB`, and
`entityIds`. `mix_plan` returns `normalizedCommands` containing the exact resolved ids. If a name or
tag is ambiguous, the batch is blocked before mutation; call `mix_resolve` or use the entity GUID.

`mix_apply` serializes competing IDE edits, rejects malformed commands before mutation, returns a semantic
diff, and automatically restores an atomic scene batch when command errors or postconditions fail. Reusing
the same `requestKey` with the same payload is safe: the engine returns the original result without spawning
or applying anything twice. Use `atomic:false` only for reviewed runtime/external side effects that cannot be
scene-snapshot rolled back.

Immediately run `{ "type": "world_report" }`. Treat its failed checks and recommendations as a
build gate, then use HELM `observe` for the render-grounded visual pass. `world_generate` remains the
lower-level terrain-only primitive when you intentionally do not want the full composer.
...
1. Place them in the `games/{{GAME_NAME}}/assets/` folder.
